...
static int loongson_canfd_chip_start(struct net_device *ndev)
{
	...
	int_msk = FIELD_GET(REG_INT_MASK_SET, ~int_ena); /* Mask all disabled interrupts */
...
static netdev_tx_t loongson_canfd_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	struct loongson_canfd_priv *priv = netdev_priv(ndev);
	u32 buf_id = LOONGSON_CANFD_TXBUF_NUM; /* invalid */
	...
	if (buf_id == LOONGSON_CANFD_TXBUF_NUM) {
		netif_stop_queue(ndev);
		netdev_err(ndev, "No writable TX buffer found, stopping queue\n");
		return NETDEV_TX_BUSY;
	}
 ...
 * Return: 1 when frame is passed to the network layer, -%EAGAIN in a case of empty Rx FIFO.
 */
static int loongson_canfd_rx(struct net_device *ndev)
{
	...
	if (meta0 & REG_FRAME_META0_XDT)
		cfd->can_id = (meta0 & CAN_EFF_MASK) | CAN_EFF_FLAG;
	else
		cfd->can_id = FIELD_GET(REG_FRAME_META0_ID_BASE, meta0) & CAN_SFF_MASK;
...
/**
 * loongson_canfd_rx_napi() - NAPI routine for rx packets
 * @napi: NAPI structure pointer
 * @quota: Max number of rx packets to be processed.
 *
 * This is the NAPI routine for rx part. It will process the packets maximux quota value.
 *
...
static int loongson_canfd_rx_napi(struct napi_struct *napi, int quota)
{
	...
	while (work_done < quota) {
		...
		if (ret == -EAGAIN) {
			/*
			 * No complete frame to read (data not ready or invalid)
			 * Assume there is no more data for now
			 */
			no_more_data = true;
			break;
		}

		/* One frame processed successfully */
		work_done++;
	}
...
static void loongson_canfd_tx_interrupt(struct net_device *ndev)
{
	...
	for (unsigned int buf_id = 0; buf_id < LOONGSON_CANFD_TXBUF_NUM; buf_id++) {
		...
		case TX_BS_CANCEL:
			...
			break;
		default:
			netdev_warn(ndev, "TXB unexpected status: %d\n", sts);
			break;
		}
...
/**
 * loongson_canfd_remove - Unregister the device after releasing the resources
