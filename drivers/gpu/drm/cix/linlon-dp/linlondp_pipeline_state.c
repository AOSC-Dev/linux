					 ...
					 struct drm_crtc *crtc)
{
	...
	if (is_switching_user(crtc, st->crtc)) {
		drm_dbg_atomic(crtc->dev, "CRTC%d required pipeline%d is busy.\n",
	...
	if (!crtc && st->active_comps) {
		drm_dbg_atomic(crtc->dev, "Disabling a busy pipeline:%d.\n", pipe->id);
		return ERR_PTR(-EBUSY);
	}
					  ...
					  void *user, struct drm_crtc *crtc)
{
	...
	if (is_switching_user(user, st->binding_user)) {
		drm_dbg_atomic(crtc->dev, "required %s is busy.\n", c->name);
		return ERR_PTR(-EBUSY);
	}
			       ...
			       struct linlondp_component_output *input, int idx)
{
	...
	if ((idx < 0) || (idx >= c->max_active_inputs)) {
		drm_dbg_atomic(c->pipeline->mdev->drm, "%s required an invalid %s-input[%d].\n",
				 input->component->name, c->name, idx);
		return -EINVAL;
	}

	if (has_bit(idx, state->active_inputs)) {
		drm_dbg_atomic(c->pipeline->mdev->drm, 
				    ...
				    struct linlondp_component_state *st)
{
	...
	if (err)
		drm_dbg_atomic(c->pipeline->mdev->drm, "%s validate private failed.\n", c->name);
	...
	struct linlondp_color_state *color_st, struct drm_plane_state *plane_st)
{
	...
	err = linlondp_color_validate(color_mgr, color_st,
				      ...
				      kplane_st->gamma_lut);
	if (err)
		drm_dbg_atomic(c->pipeline->mdev->drm, "%s validate color failed.\n", c->name);
				    ...
				    struct linlondp_data_flow_cfg *dflow)
{
	...
	if (!linlon_in_range(&layer->hsize_in, src_w)) {
		drm_dbg_atomic(kfb->base.dev, "invalidate src_w %d.\n", src_w);
		return -EINVAL;
	}

	if (!linlon_in_range(&layer->vsize_in, src_h)) {
		drm_dbg_atomic(kfb->base.dev, "invalidate src_h %d.\n", src_h);
		return -EINVAL;
	}
				     ...
				     struct linlondp_data_flow_cfg *dflow)
{
	...
	if (!linlon_in_range(&scaler->hsize, hsize_in) ||
		...
		drm_dbg_atomic(scaler->base.pipeline->mdev->drm, "Invalid horizontal sizes");
		return -EINVAL;
	}

	if (!linlon_in_range(&scaler->vsize, vsize_in) ||
	    !linlon_in_range(&scaler->vsize, vsize_out)) {
		drm_dbg_atomic(scaler->base.pipeline->mdev->drm, "Invalid vertical sizes");
		return -EINVAL;
	}
	...
	if (!scaling_ratio_valid(hsize_in, hsize_out, max_upscaling,
		...
		drm_dbg_atomic(scaler->base.pipeline->mdev->drm, "Invalid horizontal scaling ratio");
		return -EINVAL;
	}

	if (!scaling_ratio_valid(vsize_in, vsize_out, max_upscaling,
				 scaler->max_downscaling)) {
		drm_dbg_atomic(scaler->base.pipeline->mdev->drm, "Invalid vertical scaling ratio");
		return -EINVAL;
	}
				    ...
				    struct linlondp_data_flow_cfg *dflow)
{
	...
	scaler = linlondp_component_get_avail_scaler(dflow->input.component,
						     drm_st);
	if (!scaler) {
		drm_dbg_atomic(dflow->input.component->pipeline->mdev->drm, "No scaler available");
		return -EINVAL;
	}
				      ...
				      struct linlondp_data_flow_cfg *r_output)
{
	...
	if (!splitter) {
		drm_dbg_atomic(conn_st->connector->dev, "Current HW doesn't support splitter.\n");
		return -EINVAL;
	}

	if (!linlon_in_range(&splitter->hsize, dflow->in_w)) {
		drm_dbg_atomic(conn_st->connector->dev, 
				    ...
				    struct linlondp_data_flow_cfg *output)
{
	...
	if (!merger) {
		drm_dbg_atomic(kcrtc_st->base.crtc->dev, "No merger is available");
		return -EINVAL;
	}

	if (!linlon_in_range(&merger->hsize_merged, output->out_w)) {
		drm_dbg_atomic(merger->base.pipeline->mdev->drm, "merged_w: %d is out of the accepted range.\n",
				 output->out_w);
		return -EINVAL;
	}

	if (!linlon_in_range(&merger->vsize_merged, output->out_h)) {
		drm_dbg_atomic(merger->base.pipeline->mdev->drm, "merged_h: %d is out of the accepted range.\n",
				 output->out_h);
		return -EINVAL;
	}
				     ...
				     struct linlondp_data_flow_cfg *dflow)
{
	...
	if ((dflow->out_x + dflow->out_w - dflow->left_crop -
	    ...
	    dflow->out_h == 0) {
		drm_dbg_atomic(crtc->dev, "invalid disp rect [x=%d, y=%d, w=%d, h=%d]\n",
					 ...
					 struct linlondp_data_flow_cfg *dflow)
{
	...
	if (!right) {
		drm_dbg_atomic(pipe->mdev->drm, "%s has no right layer\n", left->base.name);
		return -EINVAL;
	}

	linlondp_split_data_flow(pipe->scalers[0], dflow, &l_dflow, &r_dflow);

	drm_dbg_atomic(pipe->mdev->drm, "Assign %s + %s to [PLANE:%d:%s]: ",
			 left->base.name, right->base.name,
			 plane->base.id, plane->name);

	drm_dbg_atomic(pipe->mdev->drm, "src[x/y:%d/%d, w/h:%d/%d] disp[x/y:%d/%d, w/h:%d/%d]",
				   ...
				   struct linlondp_data_flow_cfg *slave)
{
	...
	if (slave->out_x < boundary) {
		drm_dbg_atomic(kcrtc_st->base.crtc->dev,
				       ...
				       struct linlondp_data_flow_cfg *dflow)
{
	...
	drm_dbg_atomic(plane->dev, "SBS prepare %s-[PLANE:%d:%s]: %s.\n",
	...
	if (master_dflow) {
		drm_dbg_atomic(plane->dev, "SBS Master-%s assigned: %s\n",
				 master->base.name,
	...
	if (slave_dflow) {
		drm_dbg_atomic(plane->dev, "SBS Slave-%s assigned: %s\n",
				 slave->base.name,
				    ...
				    struct linlondp_data_flow_cfg *wb_dflow)
{
	...
	if (wb_dflow->en_scaling || wb_dflow->en_img_enhancement) {
		drm_dbg_atomic(conn_st->connector->dev, "sbs doesn't support WB_scaling\n");
		return -EINVAL;
	}
			      ...
			      struct drm_atomic_state *old_state)
{
	...
	drm_dbg_atomic(pipe->mdev->drm, "PIPE%d: active_comps: 0x%x, changed: 0x%lx.\n",
