					 ...
					 struct linlondp_data_flow_cfg *dflow)
{
	...
	if (dflow->blending_zorder < 0) {
		drm_dbg_atomic(pipe->mdev->drm, "%s zorder:%d < max_slave_zorder: %d.\n",
				       ...
				       struct drm_atomic_state *state)
{
	...
	if (IS_ERR(crtc_st) || !crtc_st->enable) {
		drm_dbg_atomic(kcrtc->base.dev, "Cannot update plane on a disabled CRTC.\n");
		return -EINVAL;
	}
...
static int linlondp_plane_debugfs_init(struct drm_plane *plane)
{
	...
	if (!dir) {
		drm_dbg_atomic(plane->dev, "Cannot create debugfs dir for %s\n", name);
		return 0;
	}
