...
int audit_classify_syscall(int abi, unsigned syscall)
{
	...
	default:
		if (IS_ENABLED(CONFIG_AUDITSYSCALL_O32) &&
		    (abi == AUDIT_ARCH_MIPS || abi == AUDIT_ARCH_MIPSEL))
			return audit_classify_syscall_o32(abi, syscall);
		if (IS_ENABLED(CONFIG_AUDITSYSCALL_N32) &&
		    (abi == AUDIT_ARCH_MIPS64N32 ||
		     abi == AUDIT_ARCH_MIPSEL64N32))
